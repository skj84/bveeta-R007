...
class ArduinoROS():
    def __init__(self):
        ...
        # A service to set the direction of a digital pin (0 = input, 1 = output)
        rospy.Service('~digital_set_direction', DigitalSetDirection, self.DigitalSetDirectionHandler)

        # A service to write to a digital sensor
        ...
        for name, params in sensor_params.items():
            ...
            # Create sensor objects based on the type
            if params['type'] == "Ping":
        ...
        while not rospy.is_shutdown():
            ...
            if now > self.t_next_sensors:
                ...
                for sensor in self.mySensors:
                    msg.name.append(sensor.name)
                    msg.value.append(sensor.value)
                try:
